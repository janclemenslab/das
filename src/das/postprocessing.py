...
def postprocess(class_probabilities,
                gap_dur=None,
                min_len=None,
                segment_dims=None,
                segment_names=None,
                segment_thres=None):
    probs_post = predict.predict_segments(class_probabilities,
...
def obj_fun(labels_true,
            ...
            segment_names=None):
    warnings.simplefilter("ignore")
    labels_pred = postprocess(probs_pred, gap_dur, min_len, segment_dims,
                              segment_names)
...
def optimize(
    dataset_path: str, model_save_name: str, gap_durs: List[float],
    min_lens: List[float]
    ...
    _, segments, probs_train_pred, _ = predict.predict(
    ...
    labels_train_pred = postprocess(probs_train_pred,
                                    segment_dims=segments['index'],
                                    segment_names=segments['names'])
    ...
    if len(data['val']['x']):
        # get raw predictions to run post-processing on
        _, _, probs_val_pred, _ = predict.predict(
        ...
        # get the F1 score before post-processing
        labels_val_pred = postprocess(probs_val_pred,
                                      segment_dims=segments['index'],
                                      segment_names=segments['names'])
    ...
    best_gap_dur, best_min_len = float(best_gap_dur / fs), float(best_min_len /
                                                                 fs)
